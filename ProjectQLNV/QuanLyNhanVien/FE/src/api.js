  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        console.log('API Employees Response:', response.data); // Debug: Kiểm tra dữ liệu trả về
        let employeeData;
        if (viewMode === 'all' || viewMode === 'byDepartment') {
          employeeData = Array.isArray(response.data.$values) ? response.data.$values : [];
          // Đảm bảo mỗi employee có employeeId
          employeeData = employeeData.map(emp => ({
            employeeId: emp.employeeId || null,
            fullName: `${emp.firstName || ''} ${emp.lastName || ''}` || 'Chưa có',
              ...
              {employees.length > 0 ? (
                employees.map((employee) => (
                  <tr key={employee.employeeId}>
                    <td>{employee.employeeId || 'N/A'}</td> {/* Hiển thị N/A nếu employeeId thiếu */}
                    ...
                    <td>
                      {employee.employeeId && ( // Chỉ hiển thị nút nếu có employeeId
                        <>
                          <button
