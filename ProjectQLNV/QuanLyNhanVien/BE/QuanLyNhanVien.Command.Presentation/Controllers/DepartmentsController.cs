    ...
    public class DepartmentsController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateDepartment(int departmentId, [FromBody] UpdateDepartmentCommand command)
        {
            ...
            return result.IsSuccess
                ? Ok(result)
                : result.Error.Message.Contains("Phòng ban không tồn tại")
                    ? NotFound(result)
                    : BadRequest(result);
        }
        ...
        public async Task<IActionResult> DeleteDepartment(int departmentId)
        {
            ...
            return result.Error.Message.Contains("Phòng ban không tồn tại")
                ? NotFound(result)
                : BadRequest(result);
        }
        ...
        public async Task<IActionResult> TransferDepartmentManager(int departmentId, [FromBody] TransferDepartmentManagerCommand command)
        {
            ...
            return result.IsSuccess
                ? Ok(result)
                : result.Error.Message.Contains("Department not found.") || result.Error.Message.Contains("New manager does not exist.")
                    ? NotFound(result)
                    : BadRequest(result);
        }
    }
